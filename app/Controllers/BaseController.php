...
abstract class BaseController extends Controller
{
   ...
   public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
   {
      ...
      // Load general settings with error handling
      try {
         $schoolConfigurations  = new \Config\School();
         $this->generalSettings = $schoolConfigurations::$generalSettings;
      } catch (\Exception $e) {
         // If database connection fails, set default settings
         $this->generalSettings = null;
         log_message('error', 'Failed to load general settings: ' . $e->getMessage());
      }
