...
class DataAbsenGuru extends BaseController
{
   ...
   public function ambilDataGuru()
   {
      try {
         // ambil variabel POST
         $tanggal = $this->request->getVar('tanggal') ?? Time::today()->toDateString();
         ...
         log_message('error', 'Error ambilDataGuru absen: ' . $e->getMessage());
         return '<div class="alert alert-danger">Gagal memuat data absensi guru. Silakan coba lagi.</div>';
      }
   }
