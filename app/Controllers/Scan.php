...
class Scan extends BaseController
{
   ...
   {
      // ambil variabel POST dengan sanitasi
      $uniqueCode = $this->request->getPost('unique_code');
      $waktuAbsen = $this->request->getPost('waktu');

      // Validasi input
      if (empty($uniqueCode) || empty($waktuAbsen)) {
         return $this->showErrorView('Data tidak lengkap');
      }

      // Sanitasi input
      $uniqueCode = trim($uniqueCode);
      $waktuAbsen = strtolower(trim($waktuAbsen));
   ...
   public function absenMasuk($type, $result)
   {
      ...
      if ($this->WANotificationEnabled && !empty($result['no_hp'])) {
         // Format nomor HP untuk Fonnte API (format internasional tanpa +)
         $noHp = $this->formatPhoneNumber($result['no_hp']);
   ...
   /**
    * Format nomor HP untuk Fonnte API
    * Format: 6281234567890 (internasional tanpa +, spasi, atau dash)
    * 
   ...
   protected function sendNotification($message)
   {
      try {
         ...
         if (empty($provider)) {
            log_message('warning', 'WhatsApp provider not configured');
            return;
         }
         
         if (empty($token)) {
            log_message('warning', 'WhatsApp token not configured');
            return;
         }
         ...
         switch ($provider) {
            case 'Fonnte':
               ...
               break;
            default:
               log_message('warning', 'Unknown WhatsApp provider: ' . $provider);
               return;
         }
         
         $result = $whatsapp->sendMessage($message);
         log_message('info', 'WhatsApp notification sent: ' . $result);
