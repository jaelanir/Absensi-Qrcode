   ...
   public function index()
   {
      try {
         ...
         for ($i = 6; $i >= 0; $i--) {
            ...
            try {
               $siswaHadir = $this->presensiSiswaModel
               ...
               array_push($siswaKehadiranArray, count($siswaHadir ?: []));
            } catch (\Exception $e) {
               log_message('error', 'Error fetching siswa kehadiran: ' . $e->getMessage());
               array_push($siswaKehadiranArray, 0);
            }
            
            try {
               $guruHadir = $this->presensiGuruModel
               ...
               array_push($guruKehadiranArray, count($guruHadir ?: []));
            } catch (\Exception $e) {
               log_message('error', 'Error fetching guru kehadiran: ' . $e->getMessage());
               array_push($guruKehadiranArray, 0);
            }
         }
         ...
         session()->setFlashdata('error', 'Terjadi kesalahan saat memuat dashboard. Silakan refresh halaman.');
         return view('admin/dashboard', [
            ...
            'grafikkKehadiranGuru' => [],
            'jumlahKehadiranSiswa' => ['hadir' => 0, 'sakit' => 0, 'izin' => 0, 'alfa' => 0],
            'jumlahKehadiranGuru' => ['hadir' => 0, 'sakit' => 0, 'izin' => 0, 'alfa' => 0],
            'petugas' => [],
         ]);
      }
   }
